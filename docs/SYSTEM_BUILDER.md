...
A manufacturing variable is specified the way a part is: **Tolerance ±%**
fills Minimum and Maximum around the nominal, and sets Sigma to a third of the
band so the part's rating sits at three sigma -- the same convention the
editor uses when it seeds a new variable. Because the band is a percentage of
the nominal, moving the nominal carries the limits with it. Minimum, Maximum
and Sigma remain individually editable for a model that is not a symmetric
band; a hand-edited limit that breaks the symmetry makes the percentage read
`custom`. A variable whose nominal is zero has no percentage band, so that
field is disabled and the limits are set directly.
