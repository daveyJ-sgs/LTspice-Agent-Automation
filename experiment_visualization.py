...
def _dashboard_document(
    ...
    for record in experiments:
        ...
        study = record["sampling_method"] or "deterministic"
        if record["observed_yield"] is not None:
            yield_text = f"{100 * record['observed_yield']:.2f}% aggregate"
        elif record["corner_summaries"]:
            yield_text = "; ".join(
                ",".join(
                    f"{name}={value}" for name, value in item["corners"].items()
                )
                + ": "
                + (
                    "—"
                    if item["observed_yield"] is None
                    else f"{100 * item['observed_yield']:.2f}%"
                )
                for item in record["corner_summaries"]
            )
        else:
            yield_text = "—"
        ...
        experiment_rows.append(
            ...
            f'<td>{_text(record["execution_mode"])}</td><td>{_text(study)}</td><td>{_text(yield_text)}</td>'
            f'<td>{record["finished_points"]}/{record["point_count"]}</td>'
...
<section class="panel"><h2>Experiments</h2><div class="controls"><input id="search" type="search" placeholder="Search IDs, status, method, yield, variables, requirements, or parameters" aria-label="Search experiments"><select id="status" aria-label="Filter by status"><option value="">All statuses</option>{''.join(f'<option value="{status}">{status}</option>' for status in sorted(experiment_index.EXPERIMENT_STATUSES))}</select><select id="mode" aria-label="Filter by execution mode"><option value="">All modes</option><option value="independent">independent</option><option value="native">native</option></select><span id="visible" class="muted"></span></div><div class="table-wrap"><table><thead><tr><th>Experiment</th><th>Recorded</th><th>Status</th><th>Mode</th><th>Study</th><th>Yield</th><th>Finished</th><th>Pass/fail</th><th>Parameters</th><th>Artifacts</th></tr></thead><tbody id="experiments">{''.join(experiment_rows)}</tbody></table></div></section>
